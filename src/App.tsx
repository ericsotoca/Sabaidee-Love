...
export default function App() {
  ...
  const handleToggleLanguage = () => {
    setLanguage((prev) => (prev === 'lo' ? 'fr' : 'lo'));
  };
    ...
    <div className="min-h-screen bg-stone-50 font-sans text-stone-900 selection:bg-rose-100 selection:text-rose-900">
      ...
      <div className="fixed bottom-0 inset-x-0 bg-white/95 backdrop-blur-md border-t border-stone-200 px-4 py-2 flex items-center justify-around sm:hidden z-40 shadow-lg">
        ...
        >
          ...
          <span className="text-[10px] mt-0.5">{language === 'lo' ? 'ໜ້າຫຼັກ' : 'Accueil'}</span>
        </button>
        ...
        >
          ...
          <span className="text-[10px] mt-0.5">{language === 'lo' ? 'ໂປຣໄຟລ໌' : 'Profil'}</span>
        </button>
        ...
        >
          ...
          <span className="text-[10px] mt-0.5">{language === 'lo' ? 'ວິດີໂອ' : 'Vidéo'}</span>
        </button>
        ...
        >
          ...
          <span className="text-[10px] mt-0.5">{language === 'lo' ? 'ໜຸ່ມຝຣັ່ງ' : 'Français'}</span>
        </button>
        ...
        >
          ...
          <span className="text-[10px] mt-0.5">{language === 'lo' ? 'ລະຫັດຜ່ານ' : 'Accès'}</span>
        </button>
      </div>
      ...
      <footer className="border-t border-stone-200 bg-white py-8 px-4 text-center text-xs text-stone-500 space-y-2">
        <div className="flex items-center justify-center space-x-1 font-semibold text-stone-700">
          ...
          <span>
            {language === 'lo'
              ? 'ແພລດຟອມເພື່ອການສື່ສານທີ່ຈິງໃຈ ແລະ ປອດໄພ'
              : 'Plateforme pédagogique bienveillante et respectueuse'}
          </span>
        </div>
        <p className="max-w-md mx-auto leading-relaxed">
          {language === 'lo'
            ? 'ມຸ່ງເນັ້ນການສ້າງຄວາມສຳພັນທີ່ໃຫ້ກຽດເຊິ່ງກັນ ແລະ ກັນ, ປາສະຈາກການເສແສ້ງ ແລະ ຍຶດໝັ້ນໃນຄວາມປອດໄພຂອງຜູ້ຍິງເປັນສຳຄັນ'
            : 'Favoriser des relations sincères, authentiques et équilibrées entre femmes laotiennes et hommes français.'}
        </p>
        <div className="pt-2 text-[11px] text-stone-400">
          © {new Date().getFullYear()} Sabaidee Amour • Compatible GitHub Pages & Stripe
        </div>
      </footer>
