...
export const ProfileBuilderExercise: React.FC<ProfileBuilderProps> = ({ language }) => {
  ...
  const [customName, setCustomName] = useState('Seng');
  const [customCity, setCustomCity] = useState('Vientiane');
  const [customProfession, setCustomProfession] = useState('hospitality');
  const [copied, setCopied] = useState(false);

  const hooks = [
    {
      lo: 'Sabaidee! ☀️ ຮອຍຍິ້ມອັນອົບອຸ່ນສົ່ງກົງຈາກ [ເມືອງ]. ດີໃຈຫຼາຍທີ່ໄດ້ມີໂອກາດເຮັດຄວາມຮູ້ຈັກກັນຢູ່ບ່ອນນີ້ນະເດີ້',
      en: 'Sabaidee! ☀️ A warm smile directly from {city}. Glad to welcome you to my world.',
      fr: 'Sabaidee ! ☀️ Un sourire chaleureux depuis {city}. Ravie de vous accueillir dans mon univers.'
    },
    {
      lo: 'Bonjour & Sabaidee! 🌿 ຂ້ອຍຊື່ [ຊື່], ເປັນຄົນສະບາຍໆ ທີ່ເຊື່ອໝັ້ນໃນຄວາມຈິງໃຈ ແລະ ຮອຍຍິ້ມ',
      en: 'Bonjour & Sabaidee! 🌿 I’m {name}, a positive and caring person who values true kindness.',
      fr: 'Bonjour & Sabaidee ! 🌿 Je m\'appelle {name}, une personne douce qui croit en la sincérité.'
    },
    {
      lo: 'Sabaidee! ☕ ຖ້າທ່ານມັກກາເຟຫອມໆ, ບົດສົນທະນາທີ່ສະບາຍໃຈ ແລະ ການແລກປ່ຽນວັດທະນະທຳ, ຍິນດີທີ່ໄດ້ຮູ້ຈັກເດີ້',
      en: 'Sabaidee! ☕ If you appreciate good coffee, peaceful conversations, and cultural sharing, nice to meet you!',
      fr: 'Sabaidee ! ☕ Si vous aimez le bon café, les échanges apaisants et le partage culturel, enchantée !'
    }
  ];

  const jobHobbies = [
    {
      lo: 'ຂ້ອຍເຮັດວຽກດ້ານ [ອາຊີບ], ເວລາຫວ່າງມັກເຮັດອາຫານລາວພື້ນບ້ານ, ຈັດດອກໄມ້ ແລະ ຍ່າງຫຼິ້ນຮັບລົມເຢັນໆ',
      en: 'I work in {profession}. In my free time, I love cooking traditional Lao dishes, arranging flowers, and peaceful walks.',
      fr: 'Je travaille dans {profession}. À mes heures perdues, j\'adore cuisiner des plats laos, fleurir ma maison et me promener au calme.'
    },
    {
      lo: 'ໃນຊີວິດປະຈຳວັນ, ຂ້ອຍເຮັດວຽກດ້ານ [ອາຊີບ] ແລະ ມີຄວາມສຸກກັບສິ່ງລຽບງ່າຍ ເຊັ່ນ ການຟັງເພງອະຄູສະຕິກ ແລະ ບົວລະບັດຕົ້ນໄມ້',
    ...
    {
      lo: 'ຂ້ອຍມັກການຮຽນຮູ້ສິ່ງໃໝ່ໆ, ມັກລອງຊິມອາຫານແຊບໆ ແລະ ຖ່າຍຮູບຄວາມຊົງຈຳໃນມຸມງາມໆ ຂອງເມືອງ',
  ...
  const seekings = [
    {
      lo: 'ຂ້ອຍຊອກຫາຜູ້ຊາຍທີ່ອົບອຸ່ນ, ໃຫ້ກຽດເຊິ່ງກັນ ແລະ ກັນ ເພື່ອຮ່ວມສ້າງຄວາມສຳພັນທີ່ໝັ້ນຄົງ ແລະ ເຕັມไปด้วยສຽງຫົວ',
    ...
    {
      lo: 'ຂ້ອຍໃຫ້ຄຸນຄ່າກັບຄວາມຊື່ສັດ, ການຮັບຟັງ ແລະ ການສື່ສານທີ່ກົງໄປກົງມາ ພ້ອມກ້າວເດີນໄປນຳກັນຢ່າງເຂົ້າອົກເຂົ້າໃຈ',
    ...
    {
      lo: 'ຫວັງວ່າຈະໄດ້ພົບຄົນໃຈດີ ທີ່ມັກແລກປ່ຽນມຸມມອງຊີວິດ ແລະ ພ້ອມເປີດໃຈຮຽນຮູ້ວັດທະນະທຳຂອງກັນ ແລະ ກັນ',
      en: 'Hoping to meet a kind-hearted man who enjoys sharing life stories and discovering each other’s cultures.',
      fr: 'J\'espères rencontrer un homme bienveillant qui aime échanger sur la vie et découvrir nos cultures respectives.'
    }
  ];

  const questions = [
    {
      lo: 'ແລ້ວທ່ານເດ, ກິດຈະກຳສຸດໂປດທີ່ຊ່ວຍໃຫ້ທ່ານຜ່ອນຄາຍໃນວັນຢຸດແມ່ນຫຍັງ? 😊',
    ...
    {
      lo: 'ຖ້າທ່ານມາປະເທດລາວ, ເມນູອາຫານລາວຢ່າງທຳອິດທີ່ທ່ານຢາກລອງຊິມແມ່ນຫຍັງເດ? 🍜',
      en: 'If you visit Laos, what is the first authentic Lao dish you would love to taste? 🍜',
      fr: 'Si vous venez au Laos, quel est le premier plat authentique que vous aimeriez goûter ? 🍜'
    },
    {
      lo: 'ອັນໃດຄືສິ່ງເລັກໆ ນ້ອຍໆ ໃນແຕ່ລະວັນທີ່ເຮັດໃຫ້ທ່ານຍິ້ມໄດ້ສະເໝີເດ? ☀️',
    ...
    <div className="bg-white rounded-2xl p-4 sm:p-6 border border-stone-200 shadow-sm space-y-6">
        ...
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-bold text-stone-900 text-lg">
              {language === 'lo' ? 'ແບບຝຶກຫັດປະຕິບັດ: ສ້າງໂປຣໄຟລ໌ຂອງທ່ານເທື່ອລະຂັ້ນຕອນ' : 'Atelier pratique : Rédiger sa description étape par étape'}
            </h3>
          </div>
          <p className="text-xs sm:text-sm text-stone-500 mt-1">
            {language === 'lo'
              ? 'ເລືອກຂໍ້ຄວາມ ແລະ ໃສ່ຂໍ້ມູນສ່ວນຕົວ, ລະບົບຈະລວບລວມເປັນໂປຣໄຟລ໌ພາສາອັງກິດທີ່ສຸພາບ ພ້ອມຄຳແປໃຫ້ທ່ານຄັດລອກໄປໃຊ້ໄດ້ທັນທີ'
      ...
      <div className="bg-stone-50 p-3.5 sm:p-4 rounded-xl space-y-3">
        <label className="text-xs font-semibold text-stone-700 uppercase tracking-wide">
          {language === 'lo' ? 'ຂໍ້ມູນສ່ວນຕົວຂອງທ່ານ (ປ່ຽນແປງໄດ້):' : 'Vos informations personnalisées :'}
        </label>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-white border border-stone-200 rounded-lg px-3 py-1.5 text-sm font-medium text-stone-800 focus:outline-rose-500"
              placeholder="e.g. Vientiane, Luang Prabang, Pakse"
            />
          </div>
          <div>
            <span className="text-xs text-stone-500 block mb-1">
              {language === 'lo' ? 'ຂະແໜງວຽກ (Field of work):' : 'Domaine d\'activité :'}
            </span>
            <input
              type="text"
      ...
      <div className="space-y-2">
        <div className="flex items-center space-x-2">
          ...
          <h4 className="text-sm font-semibold text-stone-900">
            {language === 'lo' ? 'ເລືອກປະໂຫຍກທັກທາຍທຳອິດ (Accroche)' : 'Étape 1 : Choisir la phrase d\'accroche'}
          </h4>
        </div>
        <div className="space-y-2">
          {hooks.map((h, i) => (
            ...
            >
              <div className="font-semibold text-stone-900 mb-0.5">
                {language === 'lo' ? h.lo.replace('[ຊື່]', customName).replace('[ເມືອງ]', customCity) : h.fr.replace('{name}', customName).replace('{city}', customCity)}
              </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center space-x-2">
          ...
          <h4 className="text-sm font-semibold text-stone-900">
            {language === 'lo' ? 'ບອກເລົ່າຕົວຕົນ ແລະ ວຽກອະດິເລກ (Personnalité & Loisirs)' : 'Étape 2 : Partager votre quotidien et vos passions'}
          </h4>
        </div>
        <div className="space-y-2">
          {jobHobbies.map((jh, i) => (
            ...
            >
              <div className="font-semibold text-stone-900 mb-0.5">
                {language === 'lo' ? jh.lo.replace('[ອາຊີບ]', customProfession) : jh.fr.replace('{profession}', customProfession)}
              </div>
              <div className="text-[11px] text-stone-500 font-mono">
      ...
      <div className="space-y-2">
        <div className="flex items-center space-x-2">
          ...
          <h4 className="text-sm font-semibold text-stone-900">
            {language === 'lo' ? 'ສິ່ງທີ່ທ່ານຊອກຫາໃນຄວາມສຳພັນ (Ce que vous recherchez)' : 'Étape 3 : Exprimer vos aspirations relationnelles'}
          </h4>
        </div>
        <div className="space-y-2">
          {seekings.map((s, i) => (
      ...
      <div className="space-y-2">
        <div className="flex items-center space-x-2">
          ...
          <h4 className="text-sm font-semibold text-stone-900">
            {language === 'lo' ? 'ຄຳຖາມທ້າຍຂໍ້ຄວາມ (Invitation & Question)' : 'Étape 4 : Poser une question engageante'}
          </h4>
        </div>
        <div className="space-y-2">
          {questions.map((q, i) => (
      ...
      <div className="mt-6 border-2 border-dashed border-rose-200 rounded-2xl p-4 sm:p-5 bg-rose-50/30">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-bold text-stone-900 text-sm sm:text-base">
              {language === 'lo' ? 'ໂປຣໄຟລ໌ສະບັບພ້ອມໃຊ້ງານຂອງທ່ານ (ພາສາອັງກິດ)' : 'Votre profil prêt à publier (version anglaise)'}
            </span>
          </div>
          <button
            onClick={handleCopy}
          ...
          >
            ...
            <span>{copied ? (language === 'lo' ? 'ຄັດລອກແລ້ວ!' : 'Copié !') : (language === 'lo' ? 'ຄັດລອກຂໍ້ຄວາມ' : 'Copier')}</span>
          </button>
        </div>
        ...
        <details className="mt-3 text-xs text-stone-600">
          <summary className="cursor-pointer font-medium text-stone-700 hover:text-stone-900 flex items-center space-x-1 py-1">
            ...
            <span>{language === 'lo' ? 'ເບິ່ງຄວາມໝາຍພາສາຝຣັ່ງ (Traduction de contrôle FR)' : 'Voir la traduction française exacte'}</span>
          </summary>
