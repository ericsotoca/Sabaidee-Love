    ...
    <div className="bg-white rounded-2xl p-4 sm:p-6 border border-stone-200 shadow-sm space-y-5">
        ...
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-bold text-stone-900 text-base sm:text-lg">
              {language === 'lo' ? title.lo : title.fr}
            </h3>
          </div>
          <p className="text-xs sm:text-sm text-stone-500 mt-1">
            {language === 'lo'
              ? 'ກວດສອບທຸກຂໍ້ເພື່ອຄວາມໝັ້ນໃຈ 100% ກ່ອນນຳໄປໃຊ້ງານຈິງ'
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex justify-between text-xs text-stone-400 font-medium">
          <span>{language === 'lo' ? `ຄວາມພ້ອມ: ${percentage}%` : `Complété : ${percentage}%`}</span>
          {isAllDone && (
            <span className="text-emerald-600 font-bold flex items-center space-x-1">
              <CheckCircle2 className="w-3.5 h-3.5" />
              <span>{language === 'lo' ? 'ພ້ອມເຜີຍແຜ່ແລ້ວ!' : 'Prêt à 100% !'}</span>
            </span>
          )}
        </div>
      </div>
        ...
        <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center space-x-3 text-emerald-900">
          ...
          <div className="text-xs sm:text-sm">
            <div className="font-bold">
              {language === 'lo' ? 'ຍິນດີນຳເດີ້! ທ່ານພ້ອມຢ່າງສົມບູນແບບແລ້ວ' : 'Félicitations ! Votre préparation est optimale.'}
            </div>
            <div className="text-emerald-800 mt-0.5">
              {language === 'lo'
                ? 'ທ່ານໄດ້ປະຕິບັດຕາມມາດຕະຖານທີ່ຖືກຕ້ອງ, ປອດໄພ ແລະ ມີສະເໜ່. ຂໍໃຫ້ພົບພໍ້ກັບຄວາມສຳພັນທີ່ດີ ແລະ ຈິງໃຈເດີ້'
