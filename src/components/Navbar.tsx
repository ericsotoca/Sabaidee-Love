    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-stone-200/80 shadow-xs">
      <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <button
          ...
          title={language === 'lo' ? 'ກັບຄືນໜ້າຫຼັກ' : 'Accueil'}
        >
          ...
          <div>
            ...
            <p className="text-[11px] text-stone-500 leading-none">
              {language === 'lo' ? 'ຄູ່ມືພັດທະນາໂປຣໄຟລ໌ & ການສື່ສານ' : 'Guide pédagogique bilingue'}
            </p>
          </div>
        </button>
        ...
        <div className="flex items-center space-x-2">
          {/* Language toggle: Bilingual control for French creator / Lao users */}
          <button
            ...
            title={language === 'lo' ? 'ເບິ່ງເປັນພາສາຝຣັ່ງ (ໂໝດຄວບຄຸມ)' : 'Passer en laotien'}
          >
            <Globe className="w-3.5 h-3.5" />
            <span>{language === 'lo' ? '🇱🇦 LO | ดู FR' : '🇫🇷 Mode FR'}</span>
          </button>
          ...
          {currentTier > 0 ? (
            ...
            >
              ...
              <span>
                {language === 'lo' ? 'ປ້ອນລະຫັດເຂົ້າຮຽນ' : 'Entrer un code'}
              </span>
            </button>
          )}
          ...
          <button
            ...
            title={language === 'lo' ? 'ຕັ້ງຄ່າລະບົບ ແລະ Stripe' : 'Administration & Stripe'}
          >
