      ...
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-b from-rose-50/80 via-white to-stone-50 p-6 sm:p-10 border border-rose-100 shadow-xs">
        <div className="max-w-2xl mx-auto text-center space-y-4">
          <div className="inline-flex items-center space-x-2 px-3 py-1.5 rounded-full bg-rose-100/70 text-rose-800 text-xs font-semibold">
            ...
            <span>
              {language === 'lo'
                ? 'ຫຼັກສູດຈິງໃຈ, ອົບອຸ່ນ ແລະ ປອດໄພສຳລັບຜູ້ຍິງລາວ'
          ...
          <h1 className="text-2xl sm:text-4xl font-extrabold text-stone-900 tracking-tight leading-tight">
            {language === 'lo' ? (
              <>
                ນຳສະເໜີຕົວຕົນຢ່າງມີສະເໜ່ ແລະ ເຂົ້າໃຈ{' '}
          ...
          <p className="text-stone-600 text-sm sm:text-base leading-relaxed">
            {language === 'lo'
              ? 'ຮຽນຮູ້ວິທີຂຽນໂປຣໄຟລ໌ໃຫ້ສະທ້ອນຕົວຕົນຈິງ, ຖ່າຍວິດີໂອແນະນຳຕົວດ້ວຍມືຖືຢ່າງໝັ້ນໃຈ ແລະ ເຂົ້າໃຈຄວາມແຕກຕ່າງທາງວັດທະນະທຳ ເພື່ອສ້າງຄວາມສຳພັນທີ່ມີຄຸນນະພາບ ແລະ ປອດໄພ'
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
            ...
            >
              <span>{language === 'lo' ? 'ເລືອກແພັກເກດຮຽນຮູ້' : 'Découvrir les 3 offres'}</span>
            ...
            >
              ...
              <span>{language === 'lo' ? 'ມີລະຫັດຢູ່ແລ້ວ? ປ້ອນຢູ່ບ່ອນນີ້' : 'J\'ai déjà mon code d\'accès'}</span>
            </button>
          </div>
        </div>
      </section>
      ...
      <section id="pricing-offers" className="space-y-6">
        <div className="text-center space-y-1.5">
          <h2 className="text-xl sm:text-2xl font-bold text-stone-900">
            {language === 'lo' ? '3 ແພັກເກດການຮຽນຮູ້ທີ່ອອກແບບມາເພື່ອທ່ານ' : '3 Offres Pédagogiques Claires & Transparentes'}
          </h2>
          <p className="text-xs sm:text-sm text-stone-500 max-w-md mx-auto">
            {language === 'lo'
              ? 'ເລືອກແພັກເກດທີ່ກົງກັບຄວາມຕ້ອງການຂອງທ່ານ, ຈ່າຍຄັ້ງດຽວ ເຂົ້າຮຽນໄດ້ຕະຫຼອດຊີບ'
              ...
              >
                ...
                {offer.lo.badge && (
                  <div className="absolute -top-3 left-6">
                    ...
                    >
                      {language === 'lo' ? offer.lo.badge : offer.fr.badge}
                    </span>
                  </div>
                )}
                ...
                <div className="space-y-2.5 flex-1 mb-6">
                  <span className="text-xs font-bold uppercase tracking-wider text-stone-700 block">
                    {language === 'lo' ? 'ສິ່ງທີ່ທ່ານຈະໄດ້ຮັບ:' : 'Contenu inclus :'}
                  </span>
                  <ul className="space-y-2 text-xs text-stone-600">
                    {(language === 'lo' ? offer.lo.features : offer.fr.features).map((feat, idx) => (
                ...
                {isUnlocked ? (
                  ...
                  >
                    ...
                    <span>{language === 'lo' ? 'ເຂົ້າສູ່ບົດຮຽນ (ປົດລັອກແລ້ວ)' : 'Accéder au module'}</span>
                  </button>
                ) : (
                  <button
                  ...
                  >
                    <span>{language === 'lo' ? offer.lo.ctaText : offer.fr.ctaText}</span>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-stone-900">
              {language === 'lo' ? 'ຫຼັກສູດທັງ 3 ໂມດູນ' : 'Programme détaillé des 3 Modules'}
            </h2>
            <p className="text-xs text-stone-500">
              {language === 'lo' ? 'ຄລິກທີ່ໂມດູນເພື່ອເບິ່ງບົດຮຽນ ແລະ ເລີ່ມຮຽນຮູ້' : 'Cliquez sur un module pour consulter ses chapitres'}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {modules.map((m, idx) => {
            ...
            const title = language === 'lo' ? m.lo.title : m.fr.title;
            const desc = language === 'lo' ? m.lo.description : m.fr.description;
            const duration = language === 'lo' ? m.lo.duration : m.fr.duration;
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {!isUnlocked && (
                        <span className="text-[10px] font-semibold bg-stone-200 text-stone-600 px-1.5 py-0.5 rounded-sm flex items-center space-x-1">
                          ...
                          <span>{language === 'lo' ? `ຕ້ອງໃຊ້ລະຫັດແພັກເກດ ${m.tierRequired}` : `Requis : Pack ${m.tierRequired}`}</span>
                        </span>
                      )}
                    </div>
                  ...
                  <div className="shrink-0">
                    {isUnlocked ? (
                      ...
                      >
                        <span>{language === 'lo' ? 'ເຂົ້າຮຽນໂມດູນນີ້' : 'Ouvrir le module'}</span>
                        <ArrowRight className="w-3.5 h-3.5" />
                      </button>
                    ) : (
                      <button
                      ...
                      >
                        ...
                        <span>{language === 'lo' ? 'ປ້ອນລະຫັດເພື່ອປົດລັອກ' : 'Débloquer avec un code'}</span>
                      </button>
                    )}
                  </div>
                </div>
      ...
      <section className="p-5 sm:p-6 rounded-3xl bg-stone-100/70 border border-stone-200/80 space-y-3">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="font-bold text-stone-900 text-sm sm:text-base">
            {language === 'lo'
              ? 'ຫຼັກການຄວາມປອດໄພ ແລະ ຄວາມຈິງໃຈຂອງພວກເຮົາ'
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs text-stone-600">
          <div className="p-3 bg-white rounded-xl border border-stone-200">
            <span className="font-bold text-stone-900 block mb-1">
              {language === 'lo' ? '1. ປອດໄພເປັນອັນດັບໜຶ່ງ' : '1. Sécurité absolue'}
            </span>
            {language === 'lo'
              ? 'ບໍ່ມີການຂໍຂໍ້ມູນທາງການເງິນ, ຫ້າມໂອນເງິນໃຫ້ຄົນທີ່ເຈີທາງເນັດເດັດຂາດ'
              : 'Protection stricte de vos données privées et règles anti-arnaques intransigeantes.'}
          </div>
          <div className="p-3 bg-white rounded-xl border border-stone-200">
            <span className="font-bold text-stone-900 block mb-1">
              {language === 'lo' ? '2. ບໍ່ມີການຊັກຈູງ ຫຼື ຫຼອກລໍ້' : '2. Zéro manipulation'}
            </span>
            {language === 'lo'
              ? 'ບໍ່ສອນໃຫ້ເສແສ້ງ, ເນັ້ນຄວາມຈິງໃຈ ແລະ ການເຄົາລົບເຊິ່ງກັນ ແລະ ກັນຢ່າງແທ້ຈິງ'
              : 'Aucune technique de séduction manipulatoire : valorisation de votre authenticité.'}
          </div>
          <div className="p-3 bg-white rounded-xl border border-stone-200">
            <span className="font-bold text-stone-900 block mb-1">
              {language === 'lo' ? '3. ເຂົ້າໃຈຄວາມຫຼາກຫຼາຍ' : '3. Respect des nuances'}
            </span>
            {language === 'lo'
              ? 'ບໍ່ເໝົາລວມຜູ້ຊາຍຝຣັ່ງ, ທຸກຄົນມີຄວາມຄິດ ແລະ ບຸກຄະລິກສະເພາະຕົວ'
